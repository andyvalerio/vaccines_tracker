...
export const parseBloodMarkers = onCall({ secrets: [geminiApiKey], region: "europe-west1" }, async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'The user must be authenticated.');
  }
  ...
  try {
    ...
    throw new HttpsError('internal', 'Failed to parse blood markers from source');
  }
});